...
public class JKTurnIn extends BaseCommandPlugin {
        ...
        dialog.showCargoPickerDialog("Select items to turn in", "Confirm", "Cancel", true, width, copy, new CargoPickerListener() {
            public void pickedCargo(CargoAPI cargo) {
                ...
                if (bounty > 0) {
                    ...
                    String soldTotalKey = "$itemValueSoldToJKThreat";
                    int curr = mem.getInt(soldTotalKey);
                    curr += bountyThreat;
                    mem.set(soldTotalKey, curr);

                    soldTotalKey = "$itemValueSoldToJKMonster";
                    curr = mem.getInt(soldTotalKey);
                    curr += bountyMonster;
                    mem.set(soldTotalKey, curr);

                    soldTotalKey = "$itemValueSoldToJKTotal";
                    curr = mem.getInt(soldTotalKey);
                    curr += bounty;
                    mem.set(soldTotalKey, curr);
                ...
                mem.set("$itemValueSoldToJKJustNowThreat", (int)bountyThreat, 0);
                mem.set("$itemValueSoldToJKJustNowMonster", (int)bountyMonster, 0);
                mem.set("$itemValueSoldToJKJustNowTotal", (int)bounty, 0);
                FireBest.fire(null, dialog, memoryMap, "JKItemsTurnedIn");
            }
