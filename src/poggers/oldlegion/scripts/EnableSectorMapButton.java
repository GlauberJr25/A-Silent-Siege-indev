...
public class EnableSectorMapButton implements EveryFrameScript {
    @Override
            ...
            if (currentTab != null) {
                ...
                if (components != null) {                                                       // Null-check in case the above fails
                    for (UIComponentAPI component : components) {                               // Iterate through all components
                        ...
                        if (sectorMapButton.isEnabled()) return;                                // Check if current button is enabled already
                        sectorMapButton.setEnabled(true);                                       // Enable ability to use button
                        break;
                    }
                }
            }
