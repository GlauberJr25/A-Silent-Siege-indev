...
public class ReflectionUtils {
    ...
    public static UIPanelAPI getCoreUI() {
        ...
        CoreUIAPI core;
        if (dialog == null) {
            core = (CoreUIAPI) ReflectionUtils.invokeMethod("getCore",campaignUI);
        }
        else {
            core = (CoreUIAPI) ReflectionUtils.invokeMethod( "getCoreUI",dialog);
        }
    ...
    public static UIPanelAPI getCoreUIForDialog() {
        ...
        if (AppDriver.getInstance().getCurrentState() instanceof CampaignState) {
            dialog = ReflectionUtils.invokeMethod("getEncounterDialog", AppDriver.getInstance().getCurrentState());
        }

        return dialog != null ? (UIPanelAPI)ReflectionUtils.invokeMethod("getParent", dialog) : null;
    }

    public static UIPanelAPI getCurrentTab() {
        UIPanelAPI coreUltimate = getCoreUI();
        UIPanelAPI core = (UIPanelAPI) ReflectionUtils.invokeMethod("getCurrentTab",coreUltimate);
        return core == null ? null : (UIPanelAPI) core;
    }
    ...
    public static UIComponentAPI getLastFromChildrenCopy(UIPanelAPI panel) {
        List<UIComponentAPI> list = getChildrenCopy(panel);
        assert list != null;
        ...
        while (currentClass != null) {
            ...
            for (Object method : methods) {
                try {
                    ...
                    MethodHandle getParameterTypesHandle = getParameterTypesHandle(method.getClass(), "getParameterTypes");
    ...
    public static List<UIComponentAPI> getChildren(UIPanelAPI panelAPI) {
        return ReflectionUtils.getChildrenCopy(panelAPI);
    }
}